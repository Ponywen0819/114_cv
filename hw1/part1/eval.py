...
def main():
    parser = argparse.ArgumentParser(
        description="evaluation function of Difference of Gaussian"
    )
    parser.add_argument(
        "--threshold",
        default=5.0,
    ...
    parser.add_argument(
        "--gt_path", default="./testdata/1_gt.npy", help="path to ground truth .npy"
    )
    ...
    if keypoints.shape[0] != keypoints_gt.shape[0]:
        print(
            "[Error] Number of KeyPoints mismatches. Yours: %d, GT: %d"
        ...
        for (x, y), (x_gt, y_gt) in zip(keypoints, keypoints_gt):
            if x != x_gt or y != y_gt:
                print(
                    "[Error] KeyPoint mismatches. Yours: (%d, %d), GT: (%d, %d)"
                    % (x, y, x_gt, y_gt)
                )
                return None
