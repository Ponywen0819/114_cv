...
def train(
    ...
    for epoch in range(cfg.epochs):
        ...
        print()
        print(
            f"[{epoch + 1}/{cfg.epochs}] {val_time:.2f} sec(s) Val Acc: {val_acc:.5f} | Val Loss: {val_loss:.5f}"
        )
...
def main():
    ...
    train_loader = get_dataloader(
        os.path.join(dataset_dir, "train"), batch_size=cfg.batch_size, split="train"
    )
    val_loader = get_dataloader(
        os.path.join(dataset_dir, "val"), batch_size=cfg.batch_size, split="val"
    )
