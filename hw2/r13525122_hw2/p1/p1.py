...
# Date: 2026-03-27
# Author: TA
# Description: BoW Scene Recognition with Tiny Images and BoW SIFT.
# ============================================================================
...
def main():
    ...
    parser.add_argument(
        "--classifier", help="classifier", type=str, default="nearest_neighbor"
    )
    parser.add_argument(
        "--dataset_dir",
    ...
    train_img_paths, test_img_paths, train_labels, test_labels = (
        get_img_paths_and_labels(args.dataset_dir)
    )
    ...
    if args.feature == "tiny_image":
        ...
        if os.path.isfile("vocab.pkl") is False:
            ...
            vocab_size = 1024  # vocab_size is up to you, larger values will work better (to a point) but be slower to compute
    ...
    accuracy = float(
        len([x for x in zip(test_labels, pred_cats) if x[0] == x[1]])
